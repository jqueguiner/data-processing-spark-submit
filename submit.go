...
var (
	args struct {
		...
		ProjectID              string   `arg:"env:OS_PROJECT_ID,required" help:"Openstack ProjectID (can be set with vars cars OS_PROJECT_ID)"`
		Version                string   `arg:"env:SPARK_VERSION" default:"2.4.3" help:"Version of spark (can be set with ENV vars SPARK_VERSION)"`
		Upload                 string   `arg:"env:UPLOAD" help:"fie path/dir to upload before run the job (can be set with ENV vars UPLOAD)"`
...
func main() {
	...
	if err := conf["ovh"].MapTo(ovhConf); err != nil {
		log.Fatalf("Unable to load \"ovh\" conf: %s", err)
	}
